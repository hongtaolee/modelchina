class AvatasController < ApplicationController
  skip_before_filter :setup_user,:log_action,:current_user, :check_code
  skip_after_filter :log_action, :remember_location 
  def show 
    @headers['Cache-Control'] = 'public'
    img = Avata.find(params[:id])
    if img
      if params[:size]
