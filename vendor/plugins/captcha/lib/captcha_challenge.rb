...
module AngryMidgetPluginsInc #:nodoc:
	...
	class CaptchaImageChallenge < CaptchaChallenge
		...
		WORDS = 'sdimdf, susfpe, banana, bender, chsuck, norris, xsrayk, vision, hearts,
				 chunky, bacson, latsex, rupert, murdoc,sdfimd,kvvins'.split(/,\s+/)
...
#			text.rotation = (rand(2)==1 ? 5 : -5) if options[:rotate]
			...
			canvas << Magick::Image.new(metric.width+options[:padding]+50, metric.height+options[:padding]){
...
#.add_noise(Magick::LaplacianNoise)
			self.image = canvas.flatten_images
		end
